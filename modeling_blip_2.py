...
class Blip2Model(Blip2PreTrainedModel):
    ...
    def __init__(self, config: Blip2Config):
        ...
        #     nn.LayerNorm(config.qformer_config.hidden_size),
        #     nn.Linear(config.qformer_config.hidden_size, config.text_config.hidden_size)
        # ])
        if config.use_decoder_only_language_model:
            language_model = AutoModelForCausalLM.from_config(config.text_config)
            language_model = AutoModelForCausalLM.from_pretrained(config.text_config._name_or_path, cache_dir=CACHE_DIR)
            for k, param in language_model.named_parameters():
    ...
    #     self.qformer.train()
    #     self.language_projection.train()
    ...
    def forward(
        ...
        inputs_embeds = self.language_model.get_input_embeddings()(input_ids)
        inputs_embeds = torch.cat([language_model_inputs, inputs_embeds], dim=1)  # DEBUGGING: COMMENT THIS TO CHECK LOSS WITHOUT PREFIX
...
class Blip2ForConditionalGeneration(Blip2PreTrainedModel):
    ...
    def __init__(self, config: Blip2Config, use_lora=False):
        super().__init__(config)

        # self.vision_model = Blip2VisionModel(config.vision_config)  # No vision model
        # self.final_norm = nn.LayerNorm(1024, eps=0.00001)
        ...
        if config.use_decoder_only_language_model:
            # language_model = AutoModelForCausalLM.from_config(config.text_config)
            print("Loading {}".format(config.text_config._name_or_path))
            if "smiles-gpt" in config.text_config._name_or_path:
                from transformers import GPT2Config, GPT2LMHeadModel, PreTrainedTokenizerFast
                language_model = GPT2LMHeadModel.from_pretrained(config.text_config._name_or_path, cache_dir=CACHE_DIR)
            else:
                language_model = AutoModelForCausalLM.from_pretrained(config.text_config._name_or_path, cache_dir=CACHE_DIR)
            ...
            # language_model = AutoModelForCausalLM.from_pretrained(config.text_config._name_or_path, cache_dir=CACHE_DIR, load_in_8bit=True, device_map='auto', low_cpu_mem_usage=True)
            # for k, param in language_model.named_parameters():
            #     param.requires_grad = False
            # language_model.eval()
        else:
            raise RuntimeError("Loading a seq2seq model instead of a decoder.")
            language_model = AutoModelForSeq2SeqLM.from_config(config.text_config)
        self.language_model = language_model
    ...
    def forward(
        ...
        # step 2: forward the query tokens through the QFormer, using the image embeddings for cross-attention
        # image_embeds = self.final_norm(image_embeds[:, None])
        ...
        inputs_embeds = self.language_model.get_input_embeddings()(input_ids)
        inputs_embeds = torch.cat([language_model_inputs, inputs_embeds.to(language_model_inputs.device)], dim=1)  # DEBUGGING: COMMENT THIS LINE

        if attention_mask is None:
            # attention_mask = torch.ones_like(input_ids)
            attention_mask = torch.zeros_like(input_ids) + (input_ids > 0)
        expected_device = language_model_attention_mask.device
        attention_mask = torch.cat([language_model_attention_mask, attention_mask.to(expected_device)], dim=1)  # DEBUGGING: COMMENT THIS LINE
        ...
        if self.config.use_decoder_only_language_model:
            ...
            if labels is not None:
                ...
                # Flatten the tokens
                loss_fct = CrossEntropyLoss(reduction="mean", ignore_index=0)
                loss = loss_fct(shift_logits.view(-1, self.config.text_config.vocab_size), shift_labels.view(-1))
        else:
            outputs = self.language_model(
                inputs_embeds=inputs_embeds,
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        Args:
            image_embeds (`torch.FloatTensor` of shape (batch_size, num_channels, height, width)):
                Input morphologies to be processed.
            input_ids (`torch.LongTensor` of shape (batch_size, sequence_length), *optional*):
